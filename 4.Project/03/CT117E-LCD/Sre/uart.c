...
void USART2_IRQHandler(void)
{
  ...
  if(USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)
  {
		...
		Usart2_Rec_Cnt =20-DMA_GetCurrDataCounter(DMA1_Channel6); //算出接本帧数据长度
		printf ("Thelenght:%d\r\n",Usart2_Rec_Cnt);
		printf ("The data:%s\r\n",RxBuffer);
		printf ("\r\nOver! \r\n");
		USART_ClearITPendingBit(USART2,USART_IT_IDLE);         		//清除中断标志
		DMA_Cmd(DMA1_Channel6, DISABLE );  //关闭USART2 TX DMA1所指示的通道    
    DMA_SetCurrDataCounter(DMA1_Channel6,20);//DMA通道的DMA缓存的大小
    DMA_Cmd(DMA1_Channel6, ENABLE);  //打开USART2 TX DMA1所指示的通道
  }
}
