...
/* System Clocks Configuration */
  ...
  /* ---------------------------------------------------------------------------
    ...
    CC1 update rate = TIM3 counter clock / CCR1_Val = 2000 Hz
    CC2 update rate = TIM3 counter clock / CCR2_Val = 1000 Hz
  ----------------------------------------------------------------------------*/
...
void TIM3_IRQHandler(void)
{
  ...
  if (TIM_GetITStatus(TIM3, TIM_IT_CC1) != RESET)
  {
    ...
    capture = TIM_GetCapture1(TIM3);
		if(!CH1_Flag)
			TIM_SetCompare1(TIM3, capture + (1.0 - CH1_Duty) * CCR1_Val );
		else
			TIM_SetCompare1(TIM3, capture + CH1_Duty * CCR1_Val );
		CH1_Flag = !CH1_Flag;			
  }
  ...
  if (TIM_GetITStatus(TIM3, TIM_IT_CC2) != RESET)
  {
    ...
    capture = TIM_GetCapture2(TIM3);
		if(!CH2_Flag)
			TIM_SetCompare2(TIM3, capture + (1.0 - CH2_Duty) * CCR2_Val );
		else
			TIM_SetCompare2(TIM3, capture + CH2_Duty * CCR2_Val );
		CH2_Flag = !CH2_Flag;			
  }
}
