...
void ADC_Channel8_Init(void)
{
  ...
  RCC_ADCCLKConfig(RCC_PCLK2_Div6); 
	...
	ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None;		/*使用的是软件触发，所以配置为ADC不使用外部触发 */
	ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;					/*右对齐，16位的数据寄存器采用右对齐的方式 */
	ADC_InitStructure.ADC_NbrOfChannel = 1;									/*扫描通道数：1 */
	ADC_Init(ADC1, &ADC_InitStructure);										/*通过结构体初始化ADC1 */

	/* ADC1 regular channel8 configuration，规则通道配置函数ADC_RegularChannelConfig */ 
